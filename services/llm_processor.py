...
class LLMProcessorService:
    ...
    async def answer_question_stream(
        self, transcript: str, question: str, history: list[dict]
    ):
        """
        ...
        """
        logger.info(
            f"Q&A question: '{question[:50]}...' "
            if len(question) > 50
            else f"Q&A question: '{question}'"
        )
        history_text = "\n".join(
            f"{h['role'].capitalize()}: {h['content']}" for h in history[-6:]
        )
